# Codex ↔ ChatGPT Collaboration Skill Issues

This public repository is the negative-issue backlog for the `codex-chat` skill: the workflow that connects Codex execution with ChatGPT planning or independent review, then returns findings to Codex for verification and fixes.
...
- Search for an existing issue first; add a recurrence there when it is the same failure mode.
- Use the [skill feedback issue form](https://github.com/atomchung/review-loop-log/issues/new/choose). Separate observed facts from cause hypotheses.
