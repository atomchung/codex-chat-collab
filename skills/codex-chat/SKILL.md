...
1. Call `cua.getState()` to identify the browser/profile for the intended signed-in ChatGPT account. For a normal handoff, open a **new browser tab and a new ChatGPT conversation** with `cua.createBrowserTab(...)`; do not bind or send into whichever ChatGPT conversation happens to be open. Existing personal chats are never a fallback target. Bind an existing tab only when the user provides its exact URL and explicitly designates that conversation for this machine task; bind it with `cua.getTab({url: exactUrl})` and verify the loaded conversation. If the intended account/profile is unclear, ask before sending.
2. Treat one independent Codex task as one machine-owned ChatGPT conversation. After the first message creates the conversation, read back and retain its exact conversation URL from the latest tab state. For a later phase of that same task (such as the PR review in a full cycle), bind an open tab by that saved URL with `cua.getTab({url: savedUrl})`; if it is not open, open that exact URL in a new tab and verify the loaded conversation. Verify its title or first-message marker before sending. Do not select by title alone. Give each concurrent or independent Codex task a separate conversation, and keep at most one in-flight request per conversation. Never reuse a personal conversation merely because it contains related project context.
